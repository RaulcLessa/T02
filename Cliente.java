...
public class Cliente {
	...
	public boolean setTimeFutebol(String timeFutebol) {
		...
		if (timeFutebol.compareTo("") == 0) {
			...
			this.timeFutebol = timeFutebol;
				...
				&& (timeFutebol
						.matches("^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ ]+$") == true)) {
			valido = true;
			this.timeFutebol = timeFutebol;
	...
	public boolean setTipoPessoa(String tipoPessoa) {
		...
		if ((tipoPessoa.compareToIgnoreCase("Fisica") == 0)
				|| (tipoPessoa.compareToIgnoreCase("Física") == 0)) {
			this.tipoPessoa = tipoPessoa;
			valido = true;
		} else if ((tipoPessoa.compareToIgnoreCase("Juridica") == 0)
				|| (tipoPessoa.compareToIgnoreCase("Jurídica") == 0)) {
			this.tipoPessoa = tipoPessoa;
			valido = true;
		} else {
			this.tipoPessoa = tipoPessoa;
	...
	public boolean setSalario(String salario) {
		...
		if (valido == true) {
			...
			if (numeroInt < 788) {
				this.salario = "Menor que um salário mínimo.";
			} else if ((numeroInt > 788) && (numeroInt < 2364)) {
				this.salario = "Entre 1 e 3 salários mínimos.";
			} else if ((numeroInt > 2364) && (numeroInt < 3940)) {
				this.salario = "Entre 3 e 5 salários mínimos.";
			} else if (numeroInt > 3940) {
				this.salario = "Maior que 5 salários mínimos.";
			}
		}
	...
	public boolean setCidade(String cidade) {
		boolean VC = cidade.matches("^([a-zA-ZáéíóÁÉÍÓÚãõÃÕÂÊâêôÔ] ?){3,50}+");
		if (VC == true) {
			this.cidade = cidade;
		}
		return VC;
	...
	public String imprimeCNPJ(String CNPJ) { // máscara do CNPJ:
		// 99.999.999.9999-99
	...
	public boolean setNome(String nome) {
		boolean validaNome = nome
				.matches("^[a-zA-ZÁÀáàÂâÃãçéèÉÈêÊíÍóòÓÒôÔõÕúÚüÜ]{1,50}+");
		if (validaNome == true) {
	...
	public boolean setProfissao(String profissao) {
		boolean validaProfissao = profissao
				.matches("^([a-zA-ZÁÀáàÂâÃãçéèÉÈêÊíÍóòÓÒôÔõÕúÚüÜ] ?){3,50}");
	...
	public static boolean validaAlfanumericos(String REF, int MAX) {
		...
		if (REF.length() <= MAX) {// maximo 50 caracteres
			...
			boolean alfanumericos = REF
					.matches("[a-zA-Z0-9_ çÇáÁéàÀÉíÍóÓúÚ]{0,50}");
	...
	public boolean setUF(String UF) {
		...
		if (UF.length() == 2) {// Apenas dois caracteres
			UF = UF.toUpperCase(); // aceito que usuário digite em minuscula
			boolean sigla = UF
